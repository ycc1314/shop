<?php /*a:2:{s:79:"/Users/seven/feisujie-master/shop/application/manage/view/user/grade_index.html";i:1567760372;s:69:"/Users/seven/feisujie-master/shop/application/manage/view/layout.html";i:1567760372;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<script>
    layui.use(['form', 'layedit', 'laydate', 'table'], function () {
        var userTables = layui.table.render({
            ...
            page: 'true',
            limit: '20',
            url: "<?php echo url('user/grade'); ?>?_ajax=1",
            id: 'userTable',
            cols: [[
        ...
        layui.form.on('submit(role-add)', function (data) {
            JsGet('<?php echo url("user/gradeAdd"); ?>', function (tpl) {
                if(tpl.status){
                    layer.open({
                        type: 1,
                        area: ['600px', '400px'],
                        ...
                        yes: function (index, layero) {
                            ...
                            JsPost('<?php echo url("user/gradeAdd"); ?>', thedata, function (re) {
                                if (re.status) {
                                    layer.msg('保存成功');
                                    layer.close(index);
                                    userTables.reload();
        ...
        $(document).on('click', '.option-del', function () {
            ...
            layer.confirm('您确定删除此会员等级吗？', {
                ...
                //去删除
                JsPost("<?php echo url('user/gradeDel'); ?>", { 'id': id }, function (res) {
                    if (res.status) {
                        layer.msg('删除成功');
                        userTables.reload();
                    } else {
        ...
        $(document).on('click', '.option-edit', function () {
            var id = $(this).attr('data-id');
            JsGet('<?php echo url("user/gradeAdd"); ?>?id=' + id, function (tpl) {
                if(tpl.status){
                    layer.open({
                        type: 1,
                        area: ['600px', '400px'],
